...
export default function FirePage() {
  ...
  const { getTodayCommonScore, getMissedCommonToday, commonHabits, getStreak, commonLogs } = useHabits()
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages:[{ role:'user', content:`You are the most intense motivational coach alive. User: ${profile?.display_name||'Warrior'}. Score: ${pct}%. Streak: ${maxStreak} days. Missed today: ${missedLabels}. Write ONE blazing motivational paragraph (4-5 sentences) that will make them want to immediately do those missed habits. Reference them by name. Sound like David Goggins meets a wise sage. Include one short power quote at the end.` }]
        })
      })
  ...
  return (
    <div className="fade-in" style={{ minHeight:'calc(100dvh - 160px)', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ background:'#0f0f0f', border:'1px solid #1e1e1e', borderLeft:'4px solid #ff5000', borderRadius:14, padding:'16px', marginBottom:14 }}>
        ...
        <div key={quoteIdx} className="fade-in" style={{ fontSize:15, color:'#ccc', lineHeight:1.7, fontStyle:'italic' }}>
          "{q.quote}"
        </div>
      ...
      {missed.length > 0 && (
        <div style={{ background:'rgba(239,68,68,0.05)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {missed.map(h => {
              const mq = MISS_QUOTES[Math.floor(Math.random()*MISS_QUOTES.length)]
              return (
                <div key={h.id} style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <span style={{ fontSize:22 }}>{h.icon}</span>
                  <div style={{ flex:1 }}>
                    <div className="bc" style={{ fontSize:15, fontWeight:800, color:'#ef4444' }}>{h.label}</div>
                    <div style={{ fontSize:11, color:'#666', fontStyle:'italic' }}>{mq.quote}</div>
                  </div>
                </div>
              )
            })}
      ...
      <div style={{ background:'#0f0f0f', border:'1px solid #1e1e1e', borderRadius:14, overflow:'hidden', marginBottom:14 }}>
        <button onClick={getAIFire} style={{
          ...
          <div style={{ borderTop:'1px solid #1a1a1a', padding:'14px 16px', background:'rgba(255,80,0,0.03)' }}>
            {aiLoading ? (
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div className="spinner"/><span style={{ fontSize:13, color:'#555' }}>Loading battle speech...</span>
              </div>
            ) : (
