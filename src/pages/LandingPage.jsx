...
const FEATURES = [
  { sym: '◈', title: 'Daily Battle Grid', desc: 'Check off habits like missions. See every day, week, and month at a glance in a visual grid — just like the reference you loved.' },
  { sym: '◉', title: 'Crew Leaderboard', desc: 'Your friends see your score only. Your specific habits stay 100% private. Compete without exposure.' },
  { sym: '◆', title: 'Private Dojo', desc: 'Add personal habits nobody sees. Side quests, spiritual practices, anything — fully yours.' },
  { sym: '▲', title: 'AI Predictions', desc: 'See what happens in 30 days if you keep grinding — and what you risk if you stop. Real projections, real stakes.' },
  { sym: '▶', title: 'Fire Zone', desc: 'Animated fire, rotating quotes from Ali, Goggins, prophets. AI battle speech before you train.' },
  { sym: '🕌', title: 'Muslim Mode', desc: 'All 5 Namaz prayers + Quran tracking built in. Deen is the ultimate discipline.' },
  { sym: '📱', title: 'Install as App', desc: 'Add to iPhone or Android home screen. Works offline. No app store needed. Pure PWA.' },
  { sym: '🔮', title: 'Predict Your Future', desc: 'AI analyses your week data and tells you exactly where you\'re heading — and how to change it.' },
]
...
// Mock calendar grid for visual demo
function MockCalendarGrid() {
  const days = Array.from({ length: 35 }, (_, i) => {
    const rand = Math.random()
    const score = i > 28 ? 0 : rand > 0.25 ? Math.floor(rand * 100) : 0
    return score
  })
...
function MockHabitRows() {
  const habits = [
    { icon: '🏋️', label: 'LIFT HEAVY', done: true, streak: 14, pct: 92 },
    { icon: '🏃', label: 'RUN 5K', done: true, streak: 7, pct: 78 },
    { icon: '💧', label: 'HYDRATE', done: true, streak: 21, pct: 95 },
    { icon: '🌙', label: 'SLEEP 8H', done: false, streak: 3, pct: 55 },
    { icon: '📚', label: 'READ 30MIN', done: false, streak: 0, pct: 40 },
  ]
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {habits.map((h, i) => (
        <div key={i} style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 14, fontWeight: 800, color: h.done ? '#f5f5f5' : '#444', letterSpacing: .5 }}>{h.label}</div>
    ...
    <div style={{ minHeight: '100dvh', background: '#0a0a0a', color: '#f5f5f5', fontFamily: "'Barlow', sans-serif", overflowX: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <img src="/icon-32.png" alt="" style={{ width: 24, height: 24, borderRadius: 5 }} onError={e => e.target.style.display = 'none'} />
          <span className="bc" style={{ fontSize: 20, fontWeight: 900, color: '#ff5000', letterSpacing: -0.5, animation: 'flicker 9s infinite' }}>HABITSTACK</span>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button onClick={() => nav('/pricing')} style={{ background: 'transparent', border: 'none', color: '#555', fontSize: 13, cursor: 'pointer', display: 'none' }}>Pricing</button>
      ...
      <div style={{ position: 'relative', zIndex: 2 }}>
        ...
        {/* ── HERO ── */}
        <section style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '100px 24px 60px', textAlign: 'center' }}>
          {/* Early bird pill */}
          ...
          <div style={{ animation: 'rise .5s ease .1s both' }}>
            <div style={{ fontSize: 72, marginBottom: 4, filter: 'drop-shadow(0 0 40px rgba(255,80,0,0.7))' }}>⚡</div>
            <div className="bc" style={{
              fontSize: 'clamp(60px,14vw,108px)', fontWeight: 900, letterSpacing: -3, lineHeight: .88,
              color: '#ff5000', animation: 'flicker 8s infinite',
              textShadow: '0 0 80px rgba(255,80,0,0.5), 0 0 160px rgba(255,80,0,0.15)',
              marginBottom: 12,
            }}>HABIT<br />STACK</div>
            <div className="bc" style={{ fontSize: 14, letterSpacing: 8, color: '#444', marginBottom: 24 }}>NO DAYS OFF</div>
          </div>

          <div style={{ animation: 'rise .5s ease .2s both', maxWidth: 560, marginBottom: 36 }}>
            <div style={{ fontSize: 'clamp(17px,3vw,22px)', color: '#888', lineHeight: 1.65, fontWeight: 300 }}>
              ...
              <br /><span style={{ color: '#ff5000' }}>Visual history grids. AI predictions. Crew leaderboard. Muslim mode.</span>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', animation: 'rise .5s ease .3s both', marginBottom: 60 }}>
          ...
          {/* LIVE MOCKUP */}
          <div style={{ width: '100%', maxWidth: 380, margin: '0 auto', animation: 'rise .6s ease .4s both' }}>
        ...
        <section style={{ padding: '0 24px 60px' }}>
          <div style={{ maxWidth: 700, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
            {STATS.map((s, i) => (
        ...
        {/* ── CALENDAR PREVIEW ── */}
        <section style={{ padding: '0 24px 70px', maxWidth: 700, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            <div className="bc" style={{ fontSize: 11, letterSpacing: 5, color: '#ff5000', marginBottom: 8 }}>INSPIRED BY YOUR REFERENCE</div>
            <div className="bc" style={{ fontSize: 'clamp(30px,6vw,52px)', fontWeight: 900, letterSpacing: -1, lineHeight: 1 }}>VISUAL HISTORY GRID</div>
            <div style={{ fontSize: 14, color: '#666', marginTop: 12, maxWidth: 420, margin: '12px auto 0', lineHeight: 1.7 }}>Every day recorded. See your consistency at a glance — past months, circle rings, habit breakdown. Exactly like that Notion tracker but smarter and live.</div>
          </div>
          <div style={{ background: '#0f0f0f', border: '1px solid #1e1e1e', borderRadius: 20, padding: '20px', boxShadow: '0 0 40px rgba(255,80,0,0.06)' }}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16 }}>
              <div className="bc" style={{ fontSize: 18, fontWeight: 900, letterSpacing: 1 }}>APRIL 2026</div>
              <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
                {['MAR','APR','MAY'].map((m, i) => <div key={i} style={{ background: i===1?'#ff5000':'#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 6, padding: '4px 10px' }}><span className="bc" style={{ fontSize: 11, fontWeight: 800, color: i===1?'#0a0a0a':'#555' }}>{m}</span></div>)}
              </div>
            </div>
            <MockCalendarGrid />
            {/* 3 circle rings */}
            <div style={{ display: 'flex', justifyContent: 'space-around', marginTop: 20, paddingTop: 16, borderTop: '1px solid #1a1a1a' }}>
                ...
                <div key={i} style={{ textAlign: 'center' }}>
                  <div style={{ position: 'relative', width: 56, height: 56, margin: '0 auto' }}>
                    <svg width="56" height="56" style={{ transform: 'rotate(-90deg)' }}>
                      ...
                      <circle cx="28" cy="28" r="22" fill="none" stroke={c} strokeWidth="5"
                        strokeDasharray={`${2*Math.PI*22}`}
                        strokeDashoffset={`${2*Math.PI*22*(1-parseInt(v)/100)}`}
                        strokeLinecap="round" style={{ filter: `drop-shadow(0 0 4px ${c})` }} />
                    </svg>
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span className="bc" style={{ fontSize: 11, fontWeight: 900, color: c }}>{v}</span>
                    </div>
                  </div>
                  <div className="bc" style={{ fontSize: 9, color: '#444', marginTop: 5, letterSpacing: 2 }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── FEATURES ── */}
        <section id="features" style={{ padding: '0 24px 70px', maxWidth: 800, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 10 }}>
            {FEATURES.map((f, i) => (
              <div key={i} className="feat-card" style={{ background: '#0f0f0f', borderRadius: 14, padding: '18px 16px' }}>
                <div className="bc" style={{ fontSize: 28, fontWeight: 900, color: '#ff5000', marginBottom: 10 }}>{f.sym}</div>
                <div className="bc" style={{ fontSize: 15, fontWeight: 800, letterSpacing: .5, marginBottom: 6 }}>{f.title}</div>
                <div style={{ fontSize: 12, color: '#666', lineHeight: 1.65 }}>{f.desc}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ── EARLY BIRD PRICING ── */}
        <section style={{ padding: '0 24px 70px', maxWidth: 560, margin: '0 auto' }}>
          <div style={{
            background: 'linear-gradient(135deg,#1e0800,#0f0f0f)', border: '2px solid #ff5000',
            borderRadius: 22, padding: '36px 28px', textAlign: 'center',
            animation: 'glow 4s infinite', position: 'relative', overflow: 'hidden',
          }}>
            ...
            <div className="bc" style={{ fontSize: 30, fontWeight: 900, color: '#ff5000', letterSpacing: -0.5, marginBottom: 8 }}>EARLY BIRD ACCESS</div>
            ...
            <div>
              <button onClick={() => nav('/auth?mode=signup')} className="bc cta-btn" style={{
                background: '#ff5000', border: 'none', color: '#0a0a0a',
                padding: '15px 38px', borderRadius: 12, fontSize: 18, fontWeight: 900, letterSpacing: 2,
                cursor: 'pointer', display: 'block', width: '100%', marginBottom: 12,
              }}>JOIN THE CREW →</button>
              <button onClick={() => nav('/pricing')} style={{ background: 'transparent', border: '1px solid #2a2a2a', color: '#555', padding: '10px 24px', borderRadius: 10, fontSize: 13, cursor: 'pointer' }}>View Pricing Details</button>
            </div>
          </div>
        </section>

        {/* ── HOW IT WORKS ── */}
        <section style={{ padding: '0 24px 70px', maxWidth: 580, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            <div className="bc" style={{ fontSize: 11, letterSpacing: 5, color: '#ff5000', marginBottom: 8 }}>PROTOCOL</div>
            <div className="bc" style={{ fontSize: 'clamp(30px,6vw,48px)', fontWeight: 900, letterSpacing: -1 }}>4 STEPS TO BEAST</div>
          </div>
          {[
            { n: '01', t: 'Sign up in 60 seconds', d: 'Pick your avatar, username. Enable Muslim mode for Namaz tracking. Zero friction.' },
            { n: '02', t: 'Load your missions', d: 'Use our proven defaults or build your own habit stack. Add private personal habits too.' },
            { n: '03', t: 'Check in every day', d: 'Tap done as you complete each habit. Watch your calendar grid fill up with orange fire.' },
            { n: '04', t: 'Compete with your crew', d: 'Invite your 6-8 friends. Leaderboard shows scores only — your habits are always private.' },
          ].map((s, i) => (
            <div key={i} style={{ background: '#0f0f0f', border: '1px solid #1e1e1e', borderLeft: '4px solid #ff5000', borderRadius: 14, padding: '18px 20px', display: 'flex', gap: 16, alignItems: 'flex-start', marginBottom: 10 }}>
              <div className="bc" style={{ fontSize: 32, fontWeight: 900, color: '#ff5000', opacity: 0.35, flexShrink: 0, lineHeight: 1, marginTop: 2 }}>{s.n}</div>
              <div>
                <div className="bc" style={{ fontSize: 18, fontWeight: 800, marginBottom: 4 }}>{s.t}</div>
                <div style={{ fontSize: 13, color: '#666', lineHeight: 1.65 }}>{s.d}</div>
              </div>
            </div>
          ))}
        </section>
        ...
        <section style={{ padding: '0 24px 80px', textAlign: 'center' }}>
          <div style={{ maxWidth: 480, margin: '0 auto' }}>
            ...
            <button onClick={() => nav('/auth?mode=signup')} className="bc cta-btn" style={{
              background: '#ff5000', border: 'none', color: '#0a0a0a',
              padding: '18px 48px', borderRadius: 14, fontSize: 22, fontWeight: 900, letterSpacing: 2,
              cursor: 'pointer', boxShadow: '0 0 50px rgba(255,80,0,0.5)', animation: 'glow 3s infinite', display: 'inline-block',
          ...
          <div style={{ maxWidth: 700, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                {[['Pricing', '/pricing'], ['Privacy Policy', '/privacy'], ['Terms of Service', '/terms'], ['Sign In', '/auth']].map(([l, p]) => (
                  <button key={l} onClick={() => nav(p)} style={{ background: 'transparent', border: 'none', color: '#555', fontSize: 13, cursor: 'pointer', transition: 'color .15s' }}
                    onMouseEnter={e => e.target.style.color = '#ff5000'}
                    onMouseLeave={e => e.target.style.color = '#555'}>{l}</button>
                ))}
              </div>
            </div>
